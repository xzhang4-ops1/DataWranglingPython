...
# truthfile_name = 'datasets/little-dirty-true-matches-100000.csv'
...
# attribute lists)
#
blocking_attrA_list = [8]
blocking_attrB_list = [8]
# optimal
...
#
# weight_vec = [1.0, 0.7, 1.0, 1.0, 1.0, 1.3,
...
print(
    '  Reduction ratio is : %.3f' % (rr) + ', Pairs completenessis is : %.3f' % (pc) + ',  Pairs quality is : %.3f' % (
        pq))
print('  Accuracy : %.3f' % (accuracy) + ',  Precision : %.3f' % (precision) + ',  Recall : %.3f' % (
    recall) + ',  F-measure : %.3f' % (fmeasure)+', Running time : %.3f sec' % (linkage_time))
